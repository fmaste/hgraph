...
newtype MapSet k v = MapSet (Map.Map k (DCSS.Set v))
...
addKey k (MapSet m) = MapSet $ Map.union m (Map.singleton k DCSS.empty)
...
addValue k v (MapSet m) = MapSet $ Map.insertWith (\new old -> DCSS.addElement v old) k (singleton v) m
	where singleton v = DCSS.addElement v $ DCSS.empty
...
removeValue k v (MapSet m) = MapSet $ Map.adjust (DCSS.removeElement v) k m
...
getValues :: (Ord k, Ord v) => k -> MapSet k v -> DCSS.Set v
getValues k (MapSet m) = Map.findWithDefault DCSS.empty k m
...
getKeysSet :: (Ord k, Ord v) => MapSet k v -> DCSS.Set k
...
getValuesList k mm = DCSS.toList $ getValues k mm

-- | The number of different values that exist for the key.
getValueCount :: (Ord k, Ord v) => k -> MapSet k v -> Int
getValueCount k mm = fromInteger $ DCSS.getElementsCount $ getValues k mm
...
containsValue k v mm = DCSS.containsElement v $ getValues k mm
...
getValuesAndRemoveKey k (MapSet m) = f $ Map.updateLookupWithKey (\_ _ -> Nothing) k m where
	...
	f (Just v, m) = (MapSet m, DCSS.toList v)
...
removeValueFromKeys ks v (MapSet m) = MapSet (Map.unionWith f m m') where
	f set _ = DCSS.removeElement v set
...
removeValuesAll k (MapSet m) = MapSet $ Map.adjust (const DCSS.empty) k m

mapSet :: (Ord k, Ord v, Ord v') => (DCSS.Set v -> DCSS.Set v') -> MapSet k v -> MapSet k v'
mapSet f (MapSet mm) = MapSet (Map.map f mm)

foldSet :: (Ord k, Ord v) => (DCSS.Set v -> ans -> ans) -> ans -> MapSet k v -> ans
foldSet f ans (MapSet mm) = Map.fold f ans mm

foldSetWithKey :: (Ord k, Ord v) => (k -> DCSS.Set v -> ans -> ans) -> ans -> MapSet k v -> ans
...
instance (Ord k, Ord v) => DC.Collection (MapSet k v) where
	...
	containsElement (k, v) m = DCSS.containsElement v $ getValues k m
	getElementsCount m = toInteger $ foldSet (\set ans -> ans + (DCSS.getElementsCount set)) 0 m

instance (Ord k, Ord v) => DC.List (MapSet k v) where
	toList m = foldSetWithKey (\k set ans -> ans ++ [(k, v) | v <- (DCSS.toList set)]) [] m
	fromList list = foldl' (\ans (k, v) -> addValue k v ans) empty list

instance (Ord k, Ord v) => DCM.Map (MapSet k v) where
	type DCM.Keys (MapSet k v) = DCSS.Set k
	type DCM.Value (MapSet k v) = DCSS.Set v
