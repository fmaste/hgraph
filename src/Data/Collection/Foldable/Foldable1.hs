...
module Data.Collection.Foldable.Foldable1 (
	Foldable1(..) ) where
...
class DCF.Foldable c => Foldable1 c where
	...
	foldr1 f c = fromMaybe (error "foldr1: empty structure") (DCF.foldr mf Nothing c) where
		mf x Nothing = Just x
	...
	foldl1 f c = fromMaybe (error "foldl1: empty structure") (DCF.foldl mf Nothing c) where
		mf Nothing y = Just y
